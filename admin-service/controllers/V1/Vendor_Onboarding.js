...
const { VendorOnboardingDB, getVendorKYCDB, getVendorDetailsDB, getVendoremployeeDB, deleteVendoremployeeDB, deleteVendorDetailsDB, deleteVendorKYCDB,
    updateVendoremployeeDB, updateVendorDetailsDB, updateVendorKYCDB, ImageUploadDB, UpdateImageUploadDB ,VehicleVerificationDB} = require('../../models/V1/Vendor_Onboarding/utility');
...
//         const vendor_id = getRandomSixDigitNumber(); // No need for stringify here
//         const vendorid = `VND${vendor_id}`;
//         console.log("Generated vendorid:", vendorid);

//         // Save vendor data to DB
//         const result = await VendorOnboardingDB(req.body, vendorid, req.body.VendorEmployeeDetails);
...
//                 // Check if userDetails is a stringified string (double quotes around JSON)
//                 if (typeof result.userDetails === "string" && result.userDetails.startsWith('"')) {
//                     userDetails = JSON.parse(JSON.parse(result.userDetails));
//                 } else {
//                     userDetails = JSON.parse(result.userDetails);
//                 }
//             }
//         } catch (parseError) {
//             console.error("❌ Error parsing userDetails:", parseError.message);
//             logger.log("error", `Error parsing userDetails: ${parseError.message}`);
//         }
...
exports.VendorOnboarding = async (req, res) => {
    ...
    try {
        ...
        // }
        logger.log("info", "Vendor Onboarding data is valid!");
        // const vendor_id = await getRandomSixDigitNumber();
...
//         // 3. Save all images and get their paths
//         const savedPaths = saveMultipleBase64Images(imagesMap, destFolder);
//         console.log('Saved files:', savedPaths);

//         // 4. Build the array of objects to insert into DB
//         const imagesToInsert = Object.entries(savedPaths).map(([key, fullPath]) => {
//             let photo_id = '';

//             switch (key) {
//                 case 'gst':
//                     photo_id = kycDetails.gstNo || 'GST';
//                     break;
//                 case 'aadhar':
//                     photo_id = kycDetails.aadharNo || 'AADHAR';
//                     break;
//                 case 'pan':
//                     photo_id = kycDetails.panNo || 'PAN';
//                     break;
//                 case 'cin':
//                     photo_id = kycDetails.cinNo || 'CIN';
//                     break;
//                 case 'cancel_cheque':
//                     photo_id = kycDetails.cancel_cheque_no || 'CHEQUE';
//                     break;
//                 case 'Vendorphoto':
//                     photo_id = 'Vendorphoto';
//                     break;
//                 default:
//                     photo_id = 'UNKNOWN';
//             }
...
exports.deleteVendoremployee = async (req, res) => {
    ...
    try {
        const result = await deleteVendoremployeeDB(vendorid);
