...
//     return new Promise((resolve, reject) => {
//         const { state, region, district, excludeDistricts } = data; 
//         // excludeDistricts = ['PUNE', 'MUMBAI'] → पहले से selected districts
...
//                     if (excludeDistricts && excludeDistricts.length > 0) {
//                         query += ` AND district NOT IN (${excludeDistricts.map((d, i) => `@dist${i}`).join(",")})`;
//                         excludeDistricts.forEach((d, i) => {
//                             request.input(`dist${i}`, sql.VarChar, d);
//                         });
//                     }
